...
class QuestionsFit extends Component {
  ...
  componentDidMount() {
    ...
    axios
      .get(`/test/startTest?moduleId=${this.props.location.state.moduleId}`, {
  ...
  render() {
    const handleAnswerButtonClick = (isCorrect, e) => {
      ...
      switch (this.state.questions[this.state.quest].type) {
        ...
        case "manyCorrect":
          ...
          var arr = [];
          var arrIsCor = this.state.questions[this.state.quest].answers.filter(
            (item) => item.isCorrect
          );
          for (let i of choice) {
            if (i.checked) {
          ...
          if (arrIsCor.length !== arr.length) {
            ...
            for (let t of choice) {
              if (t.checked) {
                if (
                  arrIsCor.findIndex(
                    (item) => item.answer._id === t.attributes["qwerty"].value
                  ) === -1
                ) {
                  zxc = false;
                }
    ...
    const handleNextQustion = () => {
      ...
      if (this.state.questions[this.state.quest].type === "write")
    ...
    const sendAnswer = () => {
      axios
        .get(`/test/closeTest?testId=${this.state.testId}`, {
          headers: { token: this.props.token },
          params: {
              ...
              <button
                ...
                onClick={(e) =>
                  handleAnswerButtonClick(
                    this.state.questions[this.state.quest].answers,
                    e
                  )
                }
              >
            ...
            <div className="footer_error">
              <div className="footer_text">
                ...
                <div className="footer_text_content">
                  <p>
                    Правильный ответ:
                    {this.state.questions[this.state.quest].answers.map(
                      (item) =>
                        item.isCorrect ? (
                          <span key={nanoid()}> {item.answer.desc}</span>
                        ) : null
                    )}
                  </p>
                </div>
              ...
              <button
                onClick={() =>
                  handleNextQustion(
                    this.state.questions[this.state.quest].rightAnswer
                  )
                }
              >
                Далее
          ...
          return arr.map((answer) => (
            <React.Fragment key={nanoid()}>
              ...
              >
                {answer.answer.img ? (
                  <img
                    src={process.env.REACT_APP_IMG_HOST + answer.answer.img}
                    alt=""
                  />
                ) : (
                  answer.answer.desc
        ...
        case "manyCorrect":
          const Checkbox = ({
            ...
            <React.Fragment>
              ...
              />
              <label htmlFor={label} qwerty={value} className={className}>
                {label}
              </label>
            </React.Fragment>
          );
                ...
                return (
                  <React.Fragment key={nanoid()}>
                    <div className="answerItem">
                      <Checkbox
                        label={item.answer.desc}
                        value={item.answer._id}
                      />
                    </div>
                  </React.Fragment>
                );
        ...
        {this.state.loader ? (
          ...
          <div className="question_block">
            <div className="progress_bar">
              ...
              <ProgressBar
                completed={
                  (this.state.progCheck * 100) / this.state.questions.length
                }
                bgColor="#45D62F"
                width="100%"
            ...
            <div className="question" id="quest">
              {/* {console.log(this.state.questions[this.state.quest])} */}
              {this.state.questions[this.state.quest].descImg ? (
                ...
                <h2 className="сhoose_correct_answer">
                  {this.state.questions[this.state.quest].desc}
                </h2>
              )}
              {/* <h2 className="сhoose_correct_answer">
                {this.state.questions[this.state.quest].descImg
                  ? this.state.questions[this.state.quest]
                  : this.state.questions[this.state.quest].desc}
              </h2> */}
