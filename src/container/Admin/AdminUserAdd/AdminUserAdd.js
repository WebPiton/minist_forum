...
import { ButtonBlue, ButtonGreen, Input, LinkBlue, Select } from "../../../StyleComponent";
...
class AdminUserAdd extends Component{
    ...
    render(){
        ...
        const addAdmin = (e) => {
            ...
            axios.get(`/user/registrationByCode?firstName=${name}&lastName=${surname}&middleName=${patronymic}&roleId=${roleId}&nickname=${nickname}&mail=${email}&password=${password}&registrationCode=${code}`,{headers: {token: this.props.token}})
                .then((res) => {
                    if (res.status === 200) {
                        swal({
        ...
        return(
            <div className="Form">
                {this.state.choice === "default" ?
                    <React.Fragment>
                        <p style={{fontSize: "22px", fontWeight: 700, marginBottom: "7px"}}>Кого добавить?</p>
                        <div>
                            <ButtonBlue onClick={() => this.setState({choice: "student"})} style={{margin: 7, width: "100%"}}>Студента</ButtonBlue>
                            <ButtonBlue onClick={() => this.setState({choice: "admin"})} style={{margin: 7, width: "100%"}}>Администратора</ButtonBlue>
                        </div>
                    </React.Fragment> : this.state.choice === "student" ?
                    <React.Fragment>
                        {this.state.studentSuccess === null ?
                            <form>
                                ...
                                <Input type="text" id="middleName" placeholder="Отчество студента"/>
                                <Input type="text" id="lastName" placeholder="Фамилия студента"/>
                            ...
                            <div className="studentSuccess">
                                {this.state.studentSuccess.map(item =>
                                    <React.Fragment key={nanoid()}>
                                        <p key={nanoid()}>Имя: <span>{item.FIO.firstName}</span></p>
                                        <p key={nanoid()}>Фамилия: <span>{item.FIO.lastName}</span></p>
                                        <p key={nanoid()}>Отчество: <span>{item.FIO.middleName}</span></p>
                                        <p key={nanoid()}>Никнейм: <span>{item.nickname}</span></p>
                                        <p key={nanoid()}>Пароль: <span>{item.password}</span></p>
                                        <p key={nanoid()}>Почта: <span>{item.mail}</span></p>
                                        <LinkBlue to="/admin" key={nanoid()}>Назад</LinkBlue>
                                    </React.Fragment>
                                )}
                            </div>
                        }
                    </React.Fragment> : this.state.choice === "admin" ?
                    <React.Fragment>
                        <form>
                            ...
                            <Select className="selectClassId" defaultValue="default">
                                ...
                                {this.state.roles.map(item => <option value={item._id} id='roleId' key={nanoid()}>{item.name}</option>)}
                            </Select>
