...
class AdminOpenModule extends Component {
  ...
  componentDidMount() {
    ...
    axios
      .get(`module/getAllCharter`, { headers: { token: this.props.token } })
      .then((res) => {
        this.setState({
  ...
  render() {
    const remWNA = (e) => {
      ...
      axios
        .get(
          `/class/remWNA?classId=${
            document.getElementById("class").value
          }&moduleIds=${JSON.stringify(arr)}`,
        ...
        <div className="Form">
          {this.state.choice === "default" ? (
            <React.Fragment>
              ...
              <div>
                <ButtonBlue
                  onClick={() => this.setState({ choice: "open" })}
                  style={{ margin: 7 }}
                >
                  Открыть модуль
                </ButtonBlue>
                <ButtonBlue
                  onClick={() => this.setState({ choice: "close" })}
                  style={{ margin: 7 }}
                >
                  Закрыть модуль
                </ButtonBlue>
